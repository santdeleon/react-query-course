    ...
    <div>
      ...
      <ul className="issues-list">
        {issues.map(({
          ...
          const tag = `#${number} opened ${relativeDate(createdDate)} by ${createdBy}`
          
          return (
            <li key={id}>
              <div>
                <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 14 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg" style={{color: "green"}}>
              ...
              <div className="issue-content">
                <span>
                  ...
                  {labels.map(({id, name, color}) => (
                    <span key={id} className={`label ${color}`}>{name}</span>
                  ))}
                </span>
                <small>{tag}</small>
              </div>
              <img className="assigned-to" src={`https://res.cloudinary.com/uidotdev/image/twitter_name/${assignee}`} alt="Assigned to avatar" />
